...
import { ScrollToTopOnMount } from "@/components/products/scroll-to-top-on-mount";
...
export default async function ProductDetailPage({ params }: ProductPageProps) {
  ...
  const useOptimizedThumbnail = thumbnailUrl.startsWith("/");
  ...
  return (
    <div className="page-container py-6 sm:py-8">
      ...
      <Link
        href="/products"
        className="mb-4 inline-flex items-center gap-2 text-sm text-zinc-500 transition-colors hover:text-zinc-300 focus-ring rounded-sm"
      >
        ...
        <div className="lg:col-span-2">
          <div
            className={cn(
              ...
              !thumbnailUrl ? getProductPlaceholderGradient(product.slug) : ""
            )}
            style={
              thumbnailUrl && !useOptimizedThumbnail
                ? {
            ...
            <div className="mt-4 grid gap-3 sm:grid-cols-2">
              {product.screenshots.map((screenshot) =>
                screenshot.startsWith("/") ? (
                  <div
                    key={screenshot}
                    className="relative aspect-video overflow-hidden rounded-md border border-[var(--border)] bg-[var(--surface)]"
                  >
                    <Image
                      src={screenshot}
